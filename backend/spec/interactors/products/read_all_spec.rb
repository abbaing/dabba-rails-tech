...
RSpec.describe Products::ReadAll do
  ...
  subject { described_class.new(company_id) }
  ...
  before do
    ...
    allow(product_presenter).to receive(:as_json).and_return(serialized_product1, serialized_product2)
  end

  describe '#call' do
    it 'returns an array of serialized products' do
      expect(subject.call).to match_array([serialized_product1, serialized_product2])
    end
  end
end
